...
require_once dirname ( __FILE__ ) . '/sync/summary2.php';
require_once dirname ( __FILE__ ) . '/sync/summary3.php';
...
function saveRecords($json_arr, $dbCon, &$response) {
	...
	/*
	 * foreach ( $json_arr ['DailyInspectionItem'] as $daily_inspection_item ) { if (! sync_daily_inspection_item ( $daily_inspection_item, $dbCon )) { $response ['error'] = array ( 'message' => "Failed to sync DailyInspectionItem records" ); return false; } } foreach ( $json_arr ['Expensedata'] as $expensedata_item ) { if (! sync_expensedata ( $expensedata_item, $dbCon )) { $response ['error'] = array ( 'message' => "Failed to sync Expensedata records" ); return false; } }
	 */
	...
	foreach ( $json_arr ['SummarySheet1'] as $summary1_item ) {
		if (! sync_summary1 ( $summary1_item, $dbCon )) {
			$response ['error'] = array (
					'message' => "Failed to sync SummarySheet1 records" 
			);
			return false;
		}
	}
	
	foreach ( $json_arr ['SummarySheet2'] as $summary2_item ) {
		if (! sync_summary2 ( $summary2_item, $dbCon )) {
			$response ['error'] = array (
					'message' => "Failed to sync SummarySheet2 records" 
			);
			return false;
		}
	}
	
	foreach ( $json_arr ['SummarySheet3'] as $summary3_item ) {
		if (! sync_summary3 ( $summary3_item, $dbCon )) {
			$response ['error'] = array (
					'message' => "Failed to sync SummarySheet3 records" 
			);
			return false;
		}
	}
